...
QQ 官方机器人当前使用的 botpy/v2 富媒体接口通常不能在一条消息里发送多张图片；群/C2C 富媒体使用 `msg_type=7` 且只携带单个 `media`，Markdown 文本使用 `msg_type=2`。程序聚合后仍会按顺序发送多张图：第一张带完整文字和链接，后续图片带“继续发送媒体 x/y”的说明，保证不会重复发送整段文字。
...
程序会优先使用 QQ 原生 markdown 消息。默认模板会把 Telegram 来源作为标题、发送者加粗展示，并在末尾追加媒体摘要和 `检测到关键词：...`。媒体摘要只显示媒体类型/数量，不显示本地缓存路径。若 QQ 官方接口返回“不允许发送原生 markdown”之类的错误，会自动使用纯文本重发。是否能真正显示 markdown 取决于 QQ 官方机器人权限和目标场景。
